...
def _run_drift_monitor_background(season: str, batch_df: pd.DataFrame, batch_id: str):
    ...
    try:
        ...
        logger.error(f"Background drift monitoring failed for batch {batch_id}: {exc}", exc_info=True)
...
async def predict_batch(request: PredictBatchRequest, background_tasks: BackgroundTasks):
    """
    ...
    try:
        ...
        if len(df) >= 5:
            background_tasks.add_task(_run_drift_monitor_background, season, df, batch_id)
        else:
