...
def get_test_record():
    ...
    feature_cols = [
        ...
        "wind_x", "wind_y", "hour_sin", "hour_cos", "day_of_week_sin", "day_of_week_cos",
...
def main() -> None:
    ...
    logger.info(f"✅ /health check passed. Active models: {[m['model_name'] for m in data['active_models']]}")
    ...
    if response.status_code == 200:
        ...
        logger.info(f"✅ /predict-batch passed. Returned {len(pred_batch['predictions'])} predictions.")
    else:
        logger.error(f"❌ /predict-batch failed: {response.status_code} - {response.text}")
        all_passed = False
